...
local st = {
    ...
    branch = "master",
    --[[tests
    mainDirUrl = "http://asv.k/asv",
    branch = "",
    mainFolder = "asv",	--if nil of false fallback into asv(folder with library)
    ]]
...
local function getLibrary(table, key)
    ...
    if st.debug.redownloadOnEachRequire or (not path and st.downloadIfNotExist) then
        if cmp.isAvailable("internet") then
            ...
            log("Internet card not found, download skipped, software exit. package: "..var[1]..key, true)
            os.exit()
        end
    end
